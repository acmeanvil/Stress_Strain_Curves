...
def display_range_component(_x_range: list(float), _y_range: list(float))->go.Figure:
    ...
    with container_1:
        ...
        with col1:
            x_range=range_slider_with_checkbox("X Range Slice", 'Select X Range Slice', _x_range)
            range_dict.update({'x_range':x_range}) 
        with col2:
            y_range=range_slider_with_checkbox("Y Range Slice", 'Select Y Range Slice', _y_range)
            range_dict.update({'y_range':y_range})  
    container_2=st.container()
    with container_2:
        slice_range=single_slider("Select Data To Display", 0.0, 100.0, (0.0,100.0))
